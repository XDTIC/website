...
const JWT_OPTIONS = {
  ...
  // subject: "xdtic-web/users",
  issuer: "WhiteRobe/xdtic-web@Github",
  expiresIn: "7d"
};
const KOA_JWT_CONFIGURE = { // JWT的解码值将会存放到 ctx.state.jwtData 中，原值将被放到 ctx.state.originToken
  secret: SERVER_PRIVATE_KEY, // JWT的签发密钥
  key: 'jwtData', // JWT解码值存放地点，请不要修改此值
  tokenKey: "originToken", // JWT原值存放地点，请不要修改此值
  // audience: (ctx) => ctx.ip, // 这两个(audience subject)值不应在此处被设置
  // subject: 'authorization',
  issuer: JWT_OPTIONS.issuer
};
const COOKIE_KEY_LIST = [SERVER_PRIVATE_KEY, 'I Like xdtic-web'];
const SESSION_AES_KEY = {
  key: "xdtic-web@Github", // length should be 16
  iv: "xdtic-web@Github", // length should be 16
};
const KOA_SESSION_CONFIGURE = { // @See https://www.npmjs.com/package/koa-session
  key: 'xdtic:sess', // (string) as cookie-key

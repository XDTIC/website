...
async function redisConnectTest() {
  ...
  await redis.set("xdtic-web.redis-connect-test", "Success!");
  try {
    await redis.get("xdtic-web.redis-connect-test", (err, result) => {
      if (err !== null) {
        let errorMsg = "Fail to connect to Redis[Error]: " + err;
        console.log(chalk.red(errorMsg));
      } else {
        console.log(chalk.green("[xdtic-web]Redis Connect Test:", result, '\n'));
      }
    });
  } catch (e) {
    throw e;
  }
  await redis.del("xdtic-web.redis-connect-test");
  await redis.disconnect();
  return {
    message: "[xdtic-web]Connect To Redis!"
  }
}
